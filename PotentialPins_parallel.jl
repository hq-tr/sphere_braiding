...
function get_Lz(all_basis::Vector{BitVector}, all_coef_ortho::Matrix{T} where T<: Number,θ::Float64, ϕ::Float64, bg_mat::SparseMatrixCSC{ComplexF64, Int64})
    ...
    @time mat = sphere_point_matrix(all_basis, θ,ϕ) + bg_mat + 5 * sparse(I, dim, dim)
...
function main()
    ...
    Threads.@threads for i in 1:npts
        println(θ_list[i])
        gap, LZ = get_Lz(all_basis, all_coef_ortho, θ_list[i], 0.0, north_pin)
        gap_list[i]= gap
        LZ_list[i] = LZ
        println("-*-*-*-*-*-")
    end
